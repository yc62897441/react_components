...
const SvgTaiwanMapWrapper = styled.div`
    position: relative;
    width: 100%;
...
const SvgWrapper = styled.div`
    ...
    path {
        stroke: white; // stroke. 指的是線、文字、及元素外框的"顏色"
        fill: transparent;
        transition: 0.5s;
        cursor: pointer;
...
// 參考來源：哲宇寇汀吧 CheYu's Coding Bar
// https://www.facebook.com/bosscodingplease/videos/1842446952675383/
// https://codepen.io/frank890417/pen/eWgPry?editors=0011
function SvgTaiwanMap() {
    ...
    useEffect(() => {
        // 在 svg 上每個縣市區塊安裝監聽器，在 mouseenter 時顯示該縣市的天氣資訊。
        const pathElements = document.querySelectorAll('path')
        pathElements.forEach((pathElement) => {
            pathElement.addEventListener('mouseenter', function () {
                const tagname = pathElement.dataset.name
                const nowPlace = place_data.filter((place) => place.tag === tagname)
                if (nowPlace.length === 0) {
                    nowPlace.push({
                        tag: null,
                        low: null,
                        high: null,
                ...
                <InfoWrapper>
                    ...
                    {nowArea.tag && (
                        <div className="forcastInfo">
                            <h2>縣市：{nowArea.tag}</h2>
                            <h4>
                                溫度：{nowArea.low}~{nowArea.high}
                            </h4>
                            <h4>
                                天氣概況：
                                {nowArea.weather}
